...
def adjoin_set(alert_code: str, alert_set: Sequence[str]) -> tuple[str, ...]:
  ...
  while low <= high:
    ...
    if alert_set[mid] == alert_code:
      ...
      high = mid - 1
  return alert_set[:low] + (alert_code,) + alert_set[low:]
...
def union_set(first_alert_set: Sequence[str], second_alert_set: Sequence[str]) -> tuple[str, ...]:
  ...
  return tuple(sorted(set(first_alert_set) | set(second_alert_set)))
...
def adjoin_leaf_set(
  ...
  for node in leaf_set:
    if not inserted and weight(leaf) < weight(node):
      result.append(leaf)
      inserted = True
    result.append(node)
...
def make_leaf_set(symbol_weight_pairs: Sequence[tuple[str, int]]) -> tuple[HuffmanLeaf, ...]:
  ...
  for symbol, wt in symbol_weight_pairs:
    leaf_set = adjoin_leaf_set(make_leaf(symbol, wt), leaf_set)
  return leaf_set
...
def generate_huffman_tree(symbol_weight_pairs: Sequence[tuple[str, int]]) -> HuffmanLeaf | HuffmanTree:
  ...
  nodes = make_leaf_set(symbol_weight_pairs)

  while len(nodes) > 1:
    left = nodes[0]
    right = nodes[1]
    merged = make_code_tree(left, right)
...
def choose_branch(bit: int, tree: HuffmanTree) -> HuffmanLeaf | HuffmanTree:
  ...
  if bit ==  0:
    ...
    raise ValueError(f"Bit must be 0 or 1 .")
...
def encode_symbol(symbol: str, tree: HuffmanLeaf | HuffmanTree) -> list[int]:
  ...
  if is_leaf(tree):
    if symbol_leaf(tree) == symbol:
      return []
    raise ValueError(f"Symbol {symbol} not in tree.")
