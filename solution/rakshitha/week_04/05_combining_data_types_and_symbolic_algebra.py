...
def make_polynomial(
    ...
    filtered_terms = tuple(
        sorted(
            (term for term in terms if term[1] != 0),
...
def add_generic(first_value: TaggedValue, second_value: TaggedValue) -> TaggedValue:
    ...
    raise TypeError(f"No add operation for {tag1} and {tag2}")
...
def mul_generic(first_value: TaggedValue, second_value: TaggedValue) -> TaggedValue:
    ...
    raise TypeError(f"No mul operation for {tag1} and {tag2}")
...
def install_plain_number_operations() -> None:
    ...
    def add_plain(first: TaggedValue, second: TaggedValue) -> TaggedValue:
        return make_plain_number(contents(first) + contents(second))

    def mul_plain(first: TaggedValue, second: TaggedValue) -> TaggedValue:
        return make_plain_number(contents(first) * contents(second))

    put_operation("add", "plain-number", "plain-number", add_plain)
    put_operation("mul", "plain-number", "plain-number", mul_plain)
...
def install_rational_operations() -> None:
    ...
    def add_rational(first: TaggedValue, second: TaggedValue) -> TaggedValue:
    ...
    def mul_rational(first: TaggedValue, second: TaggedValue) -> TaggedValue:
        return make_rational(
...
def install_polynomial_operations() -> None:
    ...
    def add_polynomial(first: TaggedValue, second: TaggedValue) -> TaggedValue:
        if polynomial_variable(first) != polynomial_variable(second):
            raise ValueError("Polynomial variables must match.")
        ...
        terms = tuple(
            sorted(
                ((order, coeff) for order, coeff in combined.items() if coeff != 0),
                reverse=True,
            )
        )

        return make_polynomial(polynomial_variable(first), terms)
...
# The extra step required was converting the RationalNumber into a normal numeric value before combining it with the polynomial result, since evaluate_polynomial() returns a float. The generic arithmetic system handled all tagged values correctly through coercions and generic operations. The only abstraction boundary crossed was after polynomial evaluation, which is expected because the function is designed to return a regular numeric result.