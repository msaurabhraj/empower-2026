...
# The extra step was converting the projected balance into a plain-number and then using the existing plain-number to rational coercion before combining it with fee_plus_rate. I could predict this from the type signatures because evaluate_polynomial returns a float while add_generic works with tagged values. I used the existing add_generic and coercion-table mechanism instead of adding any isinstance checks.