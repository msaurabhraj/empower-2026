...
def is_same_variable(first_variable: Expression, second_variable: Expression) -> bool:
  if is_variable(first_variable) and is_variable(second_variable) and first_variable ==second_variable:  # noqa: SIM103
    return True
  return False
...
def derivative(expression: Expression, variable_name: str) -> Expression:
  ...
  """ derivative answers questions like how fast? how sensitive? what is marginal increase? how does changing one thing effect other?
  think of it as a translator, calls itself
  think of derivative as a rule engine, knows exactly 4 rules, recognizes the pattern and constructs new expression
