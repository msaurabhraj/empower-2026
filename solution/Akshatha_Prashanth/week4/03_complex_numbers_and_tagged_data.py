...
def attach_tag(type_tag: str, contents: tuple[float, float]) -> Impedance:
    ...
    normalized_tag = type_tag.lower().strip()
    if normalized_tag not in ['rectangular', 'polar']:
        raise ValueError(
            f"Invalid type tag: {type_tag}. Must be 'rectangular' or 'polar'."
        )
    return (normalized_tag, contents)
...
def magnitude_rectangular(tagged_impedance: Impedance) -> float:
    ...
    resistance, reactance = contents(tagged_impedance)
...
def real_part(tagged_impedance: Impedance) -> float:
    """Returns the resistance regardless of whether tagged_impedance is rectangular- or polar-tagged."""
...
def add_impedance(first_impedance: Impedance, second_impedance: Impedance) -> Impedance:
    """Adds two impedance readings regardless of which representation each one arrived in."""
...
#Tagged data eliminates repeated isinstance/format branches wherever code must ask “which format/vendor/version is this?” — 
# Instead the check happens once in small generic selectors (e.g., attach_tag/type_tag/contents), and every other function stays representation-agnostic. 
# Examples: payment gateways returning cents-as-int vs decimal-strings, config loaders accepting parsed dicts vs env strings, or geometry code mixing vertex vs center/radius formats.