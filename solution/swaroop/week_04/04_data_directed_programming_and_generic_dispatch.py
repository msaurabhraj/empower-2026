...
def get(operation_name: str, type_tag: str) -> Callable[..., object]:
  ...
  key = (operation_name, type_tag)
  if key not in _operation_table:
    raise KeyError(f"No implementation installed for operation '{operation_name}' and type '{type_tag}'")
  return _operation_table[key]
...
def install_standard_package() -> None:
  ...
  put("cost", "standard", lambda contents: 4.0 + 0.5 * contents["weight_pounds"])
...
def install_express_package() -> None:
  ...
  put("cost", "express", lambda contents: 12.0 + 1.25 * contents["weight_pounds"])
...
def install_freight_package() -> None:
  ...
  put(
    "cost",
    "freight",
    lambda contents: contents["weight_pounds"] * (0.10 + 0.02 * contents["distance_miles"]),
  )
  put(
    "delivery-days",
    "freight",
    lambda contents: 3 + math.ceil(contents["distance_miles"] / 500.0),
  )
