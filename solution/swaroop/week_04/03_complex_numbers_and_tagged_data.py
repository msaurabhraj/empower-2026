...
def real_part_rectangular(tagged_impedance: Impedance) -> float:
  ...
  raw_resistance, _ = contents(tagged_impedance)
...
def imag_part_rectangular(tagged_impedance: Impedance) -> float:
  ...
  _, raw_reactance = contents(tagged_impedance)
  return raw_reactance
...
def real_part_polar(tagged_impedance: Impedance) -> float:
  ...
  magnitude_value, angle_value = contents(tagged_impedance)
  return magnitude_value * math.cos(angle_value)


def imag_part_polar(tagged_impedance: Impedance) -> float:
  """Returns magnitude * sin(angle) for a polar-tagged Impedance."""
  magnitude_value, angle_value = contents(tagged_impedance)
  return magnitude_value * math.sin(angle_value)
...
def real_part(tagged_impedance: Impedance) -> float:
  ...
  raise ValueError(f"Unknown impedance tag: {type_tag(tagged_impedance)}")
...
# WRITTEN ANSWER: Tagged data removes a pile of conditionals wherever the code must choose behavior based on representation before it can act. In this file, the only place a tag check is needed is inside the generic selectors `type_tag`, `contents`, `real_part`, `imag_part`, `magnitude`, and `angle`. Every other function, including `add_impedance`, stays representation-agnostic and never branches on the instrument that produced the reading.
# That same pattern is useful in a payment module that receives amount values as cents-integer from one gateway and decimal-string from another, or in a config loader that accepts both parsed YAML objects and raw environment-variable strings. Without tagging, each caller would repeat its own `isinstance` or format check and duplicate the dispatch logic. With tagged data, the representation-specific decision is centralized in one small interface layer, and the rest of the code can work against a uniform abstract value.
