...
def is_number(expression: Expression) -> bool:
  ...
  return isinstance(expression, (int, float)) and not isinstance(expression, bool)
...
def addend(sum_expression: Expression) -> Expression:
  ...
  if not is_sum(sum_expression):
    raise ValueError("Expected a sum expression")
...
def multiplier(product_expression: Expression) -> Expression:
  ...
  if not is_product(product_expression):
    raise ValueError("Expected a product expression")
...
def derivative(expression: Expression, variable_name: str) -> Expression:
  ...
  if is_product(expression):
    left_factor = multiplier(expression)
    right_factor = multiplicand(expression)
    return make_sum(
      make_product(left_factor, derivative(right_factor, variable_name)),
      make_product(right_factor, derivative(left_factor, variable_name)),
    )
