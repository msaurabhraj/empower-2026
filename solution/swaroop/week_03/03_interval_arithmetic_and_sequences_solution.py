...
def divide_interval(first_interval: Interval, second_interval: Interval) -> Interval:
  ...
  if lower_bound(second_interval) <= 0 <= upper_bound(second_interval):
    raise ValueError("Cannot divide by an interval that spans zero")
...
def parallel_resistance(resistors: list[Interval]) -> Interval:
  ...
  reciprocal_sum = make_interval(0.0, 0.0)
  for resistor in resistors:
    reciprocal = divide_interval(make_interval(1.0, 1.0), resistor)
    reciprocal_sum = add_interval(reciprocal_sum, reciprocal)

  return divide_interval(make_interval(1.0, 1.0), reciprocal_sum)
