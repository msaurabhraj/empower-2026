...
def get(operation_name: str, type_tag: str) -> Callable[..., object]:
    ...
    if key not in _operation_table:
        raise KeyError(
            f"No implementation installed for operation '{operation_name}' and type '{type_tag}'"
        )
...
def apply_generic(
    operation_name: str,
    tagged_package: tuple[str, dict[str, float]],
) -> object:
...
def install_standard_package() -> None:
    ...
    def standard_cost(contents: dict[str, float]) -> float:
        return 4.0 + (0.5 * contents["weight_pounds"])
...
def install_express_package() -> None:
    ...
    def express_cost(contents: dict[str, float]) -> float:
        return 12.0 + (1.25 * contents["weight_pounds"])
...
def install_freight_package() -> None:
    ...
    def freight_cost(contents: dict[str, float]) -> float:
        return contents["weight_pounds"] * (
            0.10 + (0.02 * contents["distance_miles"])
        )

    def freight_delivery_days(contents: dict[str, float]) -> int:
        return 3 + ceil(contents["distance_miles"] / 500)
