...
def get(operation_name: str, type_tag: str) -> Callable[..., object]:
  ...
  try:
    ...
    raise KeyError(f"No implementation found for operation '{operation_name}' with type '{type_tag}'")
...
def install_standard_package() -> None:
  ...
  put('cost', 'standard', lambda contents: 4.00 + 0.50 * contents['weight_pounds'])
...
def install_freight_package() -> None:
  ...
  put('delivery-days', 'freight', lambda contents: 3 + math.floor((contents['distance_miles'] + 499) / 500))
